import { Component, OnInit } from '@angular/core';
import { ActivatedRoute } from '@angular/router';

import { ILongonkeloHistorico } from '../longonkelo-historico.model';
import { DataUtils } from 'app/core/util/data-util.service';

@Component({
  selector: 'app-longonkelo-historico-detail',
  templateUrl: './longonkelo-historico-detail.component.html',
})
export class LongonkeloHistoricoDetailComponent implements OnInit {
  longonkeloHistorico: ILongonkeloHistorico | null = null;

  constructor(protected dataUtils: DataUtils, protected activatedRoute: ActivatedRoute) {}

  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ longonkeloHistorico }) => {
      this.longonkeloHistorico = longonkeloHistorico;
    });
  }

  byteSize(base64String: string): string {
    return this.dataUtils.byteSize(base64String);
  }

  openFile(base64String: string, contentType: string | null | undefined): void {
    this.dataUtils.openFile(base64String, contentType);
  }

  previousState(): void {
    window.history.back();
  }
}
