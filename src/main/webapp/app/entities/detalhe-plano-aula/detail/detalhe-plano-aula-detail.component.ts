import { Component, OnInit } from '@angular/core';
import { ActivatedRoute } from '@angular/router';

import { IDetalhePlanoAula } from '../detalhe-plano-aula.model';
import { DataUtils } from 'app/core/util/data-util.service';

@Component({
  selector: 'app-detalhe-plano-aula-detail',
  templateUrl: './detalhe-plano-aula-detail.component.html',
})
export class DetalhePlanoAulaDetailComponent implements OnInit {
  detalhePlanoAula: IDetalhePlanoAula | null = null;

  constructor(protected dataUtils: DataUtils, protected activatedRoute: ActivatedRoute) {}

  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ detalhePlanoAula }) => {
      this.detalhePlanoAula = detalhePlanoAula;
    });
  }

  byteSize(base64String: string): string {
    return this.dataUtils.byteSize(base64String);
  }

  openFile(base64String: string, contentType: string | null | undefined): void {
    this.dataUtils.openFile(base64String, contentType);
  }

  previousState(): void {
    window.history.back();
  }
}
