...
import { EntityArrayResponseType, ItemFacturaService } from '../service/item-factura.service';
...
export class ItemFacturaComponent implements OnInit {
  @Input() itemFacturas?: IItemFactura[];
  @Input() facturaID = 0;
  isLoading = false;
  ...
  trackId = (_index: number, item: IItemFactura): number => this.itemFacturaService.getItemFacturaIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.itemFacturas = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IItemFactura[] | null): IItemFactura[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.itemFacturaService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
