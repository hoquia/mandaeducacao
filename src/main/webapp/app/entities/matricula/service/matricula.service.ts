...
type RestOf<T extends IMatricula | NewMatricula> = Omit<T, 'timestamp'> & {
...
export class MatriculaService {
  ...
  protected convertDateFromClient<T extends IMatricula | NewMatricula | PartialUpdateMatricula>(matricula: T): RestOf<T> {
    return {
      ...matricula,
      timestamp: matricula.timestamp?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMatricula: RestMatricula): IMatricula {
    return {
      ...restMatricula,
      timestamp: restMatricula.timestamp ? dayjs(restMatricula.timestamp) : undefined,
    };
  }
  ...
  downloadListaPagoNaoPagoPdf(matriculaID: number, periodoID: number) {
    let headers = new HttpHeaders();
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    headers = headers.set('Accept', 'application/pdf');
    return this.http.get(`${this.resourceUrl}/boletim-notas/${matriculaID}/${periodoID}`, { headers, responseType: 'blob' });
  }
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type, @typescript-eslint/member-ordering
  downloadDeclaracaoSemNotas(matriculaID: number) {
    let headers = new HttpHeaders();
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    headers = headers.set('Accept', 'application/pdf');
    return this.http.get(`${this.resourceUrl}/declaracao/${matriculaID}`, { headers, responseType: 'blob' });
  }
