...
export class DocenteUpdateComponent implements OnInit {
  ...
  grauAcademicoCollection: ILookupItem[] = [];
  categoriaProfissionalCollection: ILookupItem[] = [];
  unidadeOrganicaCollection: ILookupItem[] = [];
  estadoCivilCollection: ILookupItem[] = [];
  ...
  protected loadRelationshipsOptions(): void {
    ...
    // grauAcademico
    this.lookupItemService.query({ 'lookupId.equals': 972 }).subscribe(res => {
      this.grauAcademicoCollection = res.body ?? [];
    });
    // unidadeOrganica
    this.lookupItemService.query({ 'lookupId.equals': 974 }).subscribe(res => {
      this.unidadeOrganicaCollection = res.body ?? [];
    });
    // estadoCivil
    this.lookupItemService.query({ 'lookupId.equals': 3 }).subscribe(res => {
      this.estadoCivilCollection = res.body ?? [];
    });
