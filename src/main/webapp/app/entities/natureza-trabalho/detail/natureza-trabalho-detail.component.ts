import { Component, OnInit } from '@angular/core';
import { ActivatedRoute } from '@angular/router';

import { INaturezaTrabalho } from '../natureza-trabalho.model';
import { DataUtils } from 'app/core/util/data-util.service';

@Component({
  selector: 'app-natureza-trabalho-detail',
  templateUrl: './natureza-trabalho-detail.component.html',
})
export class NaturezaTrabalhoDetailComponent implements OnInit {
  naturezaTrabalho: INaturezaTrabalho | null = null;

  constructor(protected dataUtils: DataUtils, protected activatedRoute: ActivatedRoute) {}

  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ naturezaTrabalho }) => {
      this.naturezaTrabalho = naturezaTrabalho;
    });
  }

  byteSize(base64String: string): string {
    return this.dataUtils.byteSize(base64String);
  }

  openFile(base64String: string, contentType: string | null | undefined): void {
    this.dataUtils.openFile(base64String, contentType);
  }

  previousState(): void {
    window.history.back();
  }
}
