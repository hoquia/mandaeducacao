...
public class HorarioDiscenteReport {
    ...
    private String getInfoEmpresa() {
        ...
        var departamento = "Sub-Direção pedagógica";

        var nif = "Nif nº " + instituicao.getNif();
        var tituloMapa = "Horário de Turma";
    ...
    private PdfPTable getHorarioDiscenteDetalhe(Long turmaID) {
        ...
        if (segundoTempo.isPresent()) {
            tableDetalhe =
                getHorarioTempo(horarios, segundoTempo.get().getPeriodo(), tableDetalhe, tableFontNormal, leading, padding, border);
        }

        if (terceiroTempo.isPresent()) {
            tableDetalhe =
                getHorarioTempo(horarios, terceiroTempo.get().getPeriodo(), tableDetalhe, tableFontNormal, leading, padding, border);
        }

        if (quartoTempo.isPresent()) {
            tableDetalhe =
                getHorarioTempo(horarios, quartoTempo.get().getPeriodo(), tableDetalhe, tableFontNormal, leading, padding, border);
        }

        if (quintoTempo.isPresent()) {
            tableDetalhe =
                getHorarioTempo(horarios, quintoTempo.get().getPeriodo(), tableDetalhe, tableFontNormal, leading, padding, border);
        }

        if (sextoTempo.isPresent()) {
            tableDetalhe =
                getHorarioTempo(horarios, sextoTempo.get().getPeriodo(), tableDetalhe, tableFontNormal, leading, padding, border);
        }
